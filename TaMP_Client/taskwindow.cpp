...
/**
 * @file taskwindow.cpp
 * @brief Реализация окна TaskWindow с интерактивным графиком и таблицей.
 */

/**
 * @brief Создаёт все виджеты, настраивает layout и выполняет первоначальную отрисовку.
 */
...
/**
 * @brief Настраивает внешний вид QCustomPlot: цвета, оси, три графика, легенду.
 *
 * @details Каждый из трёх графиков (graph 0/1/2) соответствует одной ветви функции:
 * синий — x < 0, зелёный — 0 ≤ x < 2, красный — x ≥ 2.
 * Включены зум и перетаскивание (QCP::iRangeDrag | QCP::iRangeZoom).
 */
...
/**
 * @brief Считывает значения слайдеров, пересчитывает три ветви функции и перерисовывает график.
 *
 * @details Для каждой ветви функции генерируется набор из N=500 точек на
 * соответствующем интервале. Данные передаются в графики 0, 1, 2 QCustomPlot.
 * После перерисовки вызывается updateTable().
 */
